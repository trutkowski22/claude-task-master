/**
 * remove-dependency-db.js
 * Database-powered implementation for removing a dependency from a task
 * 
 * This replaces the file-based remove-dependency.js with database operations
 */
...
/**
 * Database-powered dependency removal function
 * @param {string} userId - User ID (extracted from auth context)
 * @param {string|number} taskId - Task ID to remove dependency from
 * @param {string|number} dependsOnId - Task ID to remove as a dependency
 * @param {Object} options - Dependency removal options
 ...
 * @returns {Promise<Object>} Dependency removal result
 */
async function removeDependencyDb(
    userId,
    taskId,
    dependsOnId,
    options = {},
    ...
    try {
        ...
        logFn.info(`Removing dependency: task ${taskId} will no longer depend on ${dependsOnId}`);
        ...
        // Get dependency task (to validate it exists)
        try {
        ...
        // For subtasks, we store dependencies on the parent task level
        ...
        if (!existingDependencies.includes(actualDependencyTaskId)) {
            throw new DatabaseError(`Task #${targetTask.taskNumber} does not depend on task #${dependencyTask.taskNumber}`);
        }

        // Remove the dependency
        const updatedDependencies = existingDependencies.filter(depId => depId !== actualDependencyTaskId);
        
        // Update the task without the dependency
        await db.tasks.update(userId, actualTargetTaskId, {
            dependencies: updatedDependencies
        });

        logFn.info(`Successfully removed dependency: Task #${targetTask.taskNumber} no longer depends on #${dependencyTask.taskNumber}`);

        // Log dependency removal in history
        await db.history.log(userId, {
            action: 'dependency_removed',
            changeSummary: `Removed dependency: Task #${targetTask.taskNumber} no longer depends on #${dependencyTask.taskNumber}`,
            ...
            newValue: {
                ...
                removedDependency: dependencyTask.taskNumber,
                dependencies: updatedDependencies
            }
        });

        const successMessage = `Successfully removed dependency: Task #${targetTask.taskNumber} no longer depends on #${dependencyTask.taskNumber}`;
        ...
        logFn.error(`Error removing dependency: ${error.message}`);
        
        throw new DatabaseError(`Failed to remove dependency: ${error.message}`, error.code, {
            taskId,
            dependsOnId,
            options,
...
/**
 * Direct function wrapper for removing dependencies with database operations
 * 
...
export async function removeDependencyDirect(args, log, context = {}) {
    ...
    try {
        ...
        mcpLog.info(`Removing dependency: task ${id} no longer depends on ${dependsOn}, ProjectRoot: ${projectRoot}`);

        // Call the database-powered dependency removal
        const result = await removeDependencyDb(
            userId,
            id,
            dependsOn,
            {
        ...
        mcpLog.error(`Error in removeDependencyDirect: ${error.message}`);
        return {
            success: false,
            error: {
                code: error.code || 'REMOVE_DEPENDENCY_DB_ERROR',
                message: error.message || 'Unknown error removing dependency',
